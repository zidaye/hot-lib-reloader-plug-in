...
pub fn generate_lib_reloader_struct(
    ...
    Ok(quote::quote! {
        pub struct #name {
            lib_loader: HotLoadingManager,
        }

        impl #name {
            pub fn new() -> Result<Self, ::hot_lib_reloader_plug_in::HotReloaderError> {
                Ok(Self {
                    lib_loader: HotLoadingManager::new(#lib_dir, #lib_name, None)?,
                })
            }

            /// Checks if the watched library has changed. If it has, reload it and return
            /// true. Otherwise return false.
            pub fn update(&mut self) -> Result<bool, ::hot_lib_reloader_plug_in::HotReloaderError> {
...
/// body for the HotLoadingManager struct that calls the library function.
///
/// 2. It generates a function signature that can be used as signature of a
/// method for the specific HotLoadingManager struct.
///
